...
export interface GmailTracker<T extends string = never> {
...
export type GmailDomComposeRecipients = {
  to: string[];
  cc: string[];
  bcc: string[];
}

export type GmailAttachmentDetails = {
...
export type GmailLoggedInAccount = {
  name: string,
  email: string,
  index: number
};

export type GmailStorageInfo = {
  used: string,
  total: string,
  percent: number
};

export interface GmailGet {
  /**
...
export interface GmailDomEmailEntry {
  el?: JQuery,
  email: string,
  name: string
}

export type GmailDomThreadLookup = "opened_email" | "subject" | "labels";

export interface GmailDomThread {
  $el: JQuery,
  /**
...
export interface GmailDomAttachment {
  $el: JQuery,
  type?: string,
  name: string,
  size: string,
...
export type GmailDomComposeLookup =
...
export interface GmailDomCompose {
  $el: JQuery,
  /**
...
export type GmailHttpRequestMethod = "GET" | "POST";
...
export type GmailComposeType = "reply" | "forward" | "compose";
export type GmailBindType = 'on' | 'before' | 'after';
export type GmailBindAction =
  ...
  | 'load_email_menu';
export type GmailDomObserver =
  'view_thread' | 'view_email' | 'load_email_menu' | 'recipient_change' | 'compose'

export interface HttpEventRequestParams {
  url: object,
  url_raw: string;
  body: string;
  body_params: object;
...
export interface GmailObserve<T extends string = never> {
  /**
...
export interface GmailNewEmailAddress {
  name: string;
  address: string;
}

export interface GmailNewEmailData {
  id: string;
...
export interface GmailSentEmailData {
  1: string;
  id: string;
  subject: string;
  timestamp: number;
...
export interface GmailNewThreadData {
  thread_id: string;
  emails: GmailNewEmailData[];
}

export interface GmailNewGet {
  /**
...
export interface GmailNew {
  get: GmailNewGet;
}

export interface GmailCache {
...
export declare class Gmail<T extends string = never> {
